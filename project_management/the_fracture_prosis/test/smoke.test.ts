...
// ---- ROLES data ----test("ROLES exposes 12 abilities across 4 cost-shapes", () => {
...
test("computeGTL: BANKING_GTL_CREDIT flows into chosenFront", () => {
  ...
  assert.strictEqual(without.chosenFront, "systems"); // systems has the highest unmet deficit
  assert.strictEqual(withBanking.chosenFront, "re");
});

// ---- Reveal ----
test("computeReveal: rng returning a low value produces a lull (rng< lullChance)", () => {
  const lowRng = () => 0.05; // < lullChance at entropy0 (~0.46)
...
test("stepRicky: zeros belief on bad call (gapMagnitude > BAD_CALL_THRESHOLD)", () => {
  const bad = computeGTL(1, { entropy: 0, systems: 100, re: 100 }, { entropy: 0, systems: 50, re: 0 });
  // Pick the case where rankOfChosen / (n-1) > BAD_CALL_THRESHOLD
  const forceBad: typeof bad = { ...bad, gapMagnitude: BAD_CALL_THRESHOLD + 0.1 };
...
test("stepRicky: builds belief on a good call", () => {
  const good: ReturnType<typeof computeGTL> = {
...
test("stepDez: returns defianceChance > 0 when trust < distrustThreshold", () => {
  const gtl: ReturnType<typeof computeGTL> = {
    optimalFront: "entropy",
    chosenFront: "entropy",
    gapMagnitude: 0.3,
    levelEfficiency: 0.7,
...
test("maudeTax: 0 →0, grace rounds → base tax, beyond grace → scales", () => {
  ...
  assert.strictEqual(maudeTax(3), 0.25); // grace rounds
  assert.ok(maudeTax(10) > 0.25);
  assert.ok(maudeTax(100) <= 0.70); // capped
});
...
test("resolveDezRound: never fires when optimalFront !== 're'", () => {
  ...
  const result = resolveDezRound(-5, gtl, { abilityId: "force_extraction", level: "I" }, () => 0.5);
...
test("resolveDezRound: fires when optimalFront === 're' and roll< defianceChance", () => {
  ...
  const result = resolveDezRound(-5, gtl, { abilityId: "force_extraction", level: "I" }, () => 0);
...
test("tickBankGrowth: locks growth after BANK_GROWTH_CAP_ROUNDS", () => {
  ...
  // After cap, banked should equal the value at exactly the cap round.
  // Just verify it stops compounding past cap — compare to one more tick.
...
test("pressureForMoraleDrain: morale low alone does NOT trigger drain", () => {
  // The bug-fix invariant: low morale must not bootstrap its own drain.
  assert.strictEqual(
...
test("applySalvageAutoSpend: weak front gets the restoration", () => {
  ...
  // RE is the weakest (50/100), so restoration goes to re.
...
// (Threat + banking in one round, via deterministic RNG)
test("integration: different-front + same-front-later + banking in one round produces expected state shape", () => {
  ...
  // Sanity: Force the Correction → entropy -1.5, systems -3.
  assert.deepStrictEqual(helmAbility.levels.I.entropyDelta, -1.5);
  assert.deepStrictEqual(helmAbility.levels.I.systemsDelta, -3);
  // Overclock → systems +6, wearBump 0.02 / 2 rounds.
  assert.strictEqual(engAbility.levels.I.systemsDelta, 6);
  assert.strictEqual(engAbility.levels.I.wearBump, 0.02);
  // Stockpile → banked 4, front re.
